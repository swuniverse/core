...
import { ArrowLeft, MapPin, Users, Zap, Trash2, Rocket, X, Coins, Wrench, Gem, Factory, Clock } from 'lucide-react';
...
export default function Planet() {
  ...
  const [activeTab, setActiveTab] = useState<'orbit' | 'surface' | 'underground'>('surface');
  ...
  const renderGrid = (fields: PlanetField[][], layerName: string) => {
    return (
  ...
  return (
    <div className="pb-20 md:pb-0">
      {/* Mobile Sticky Resource Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 bg-space-dark border-b border-gray-700 z-40 px-4 py-2">
        <div className="flex items-center justify-between gap-2 text-xs">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-white font-mono">{planet.energyStorage}/{planet.energyStorageCapacity}</span>
          </div>
        </div>
      </div>
