...
for i in $(seq 1 60); do
    if docker compose -f docker-compose.prod.yml exec -T backend wget --no-verbose --tries=1 --spider http://localhost:3001/api/health 2>/dev/null; then
...
docker compose -f docker-compose.prod.yml -f docker-compose.migrate.yml run --rm migrate || echo -e "${YELLOW}Migration fehlgeschlagen (manuell pruefen)${NC}"
echo ""
