  ...
  async seedDefaults(): Promise<void> {
    ...
    for (const def of yamlDefs) {
      const definition = {
        ...
        reactorBase: def.reactorBase ?? 0,
        warpdriveBase: def.warpdriveBase ?? def.warpBase,
        evadeBase: def.evadeBase ?? 0,
        hitChanceBase: def.hitChanceBase ?? 75,
        sensorRangeBase: def.sensorRangeBase ?? 2,
