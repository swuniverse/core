...
import { fieldTypeAsset, getFieldTypeClasses } from './utils';
...
export function FieldEditor() {
  const {
    ...
    createSystemForm,
    setCreateSystemForm,
    createSystemForSelectedField,
    regenerateSelectedSystem,
    initializeSelectedSystemGrid,
    openSystem,
    systemFields,
    systemGridFieldTypeId,
    setSystemGridFieldTypeId,
  } = useStarmapAdminStore();

  const systemGridExists = systemFields.length > 0;
  const selectedAsset = fieldTypeAsset(selectedField?.fieldType);
    ...
    return (
      <div className="rounded-lg border border-swu-border bg-swu-surface p-4">
        ...
        <p className="mt-3 text-sm text-swu-muted">
          Feld in der Kartenansicht anklicken.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-swu-border bg-swu-surface p-4">
      ...
      <div className="mt-3 space-y-4 text-sm">
        <div className="flex items-start gap-3 rounded border border-swu-border/60 bg-swu-bg/40 p-3">
          <div
            className={[
              'relative h-12 w-12 shrink-0 overflow-hidden rounded border bg-swu-surface',
              getFieldTypeClasses(selectedField.fieldType),
            ].join(' ')}
          >
            {selectedAsset && (
              <img
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-swu-muted">
              Systemtyp-Seed:{' '}
              {systemTypes.find((st) => st.id === selectedField.systemTypeId)
                ?.name || '—'}
            </div>
          </div>
        </div>
        ...
        <label className="block text-xs text-swu-muted">
          ...
          <select
            ...
            onChange={(e) =>
              void updateField(selectedField.id, {
                factionZone: e.target
                  .value as StarmapGalaxyFieldDto['factionZone'],
              })
            }
            className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
          >
            {['UNKNOWN', 'REBEL', 'EMPIRE', 'CONTESTED', 'NEUTRAL'].map((z) => (
        ...
        <label className="block text-xs text-swu-muted">
          Systemtyp-Seed
          <select
            value={selectedField.systemTypeId ?? ''}
            onChange={(e) =>
              void updateField(selectedField.id, {
                systemTypeId: e.target.value ? Number(e.target.value) : null,
              })
            }
            className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
          >
        ...
        {borderTypes.length > 0 && (
          <label className="block text-xs text-swu-muted">
            Border-Typ
            <select
              value={selectedField.borderTypeId ?? ''}
              onChange={(e) =>
                void updateField(selectedField.id, {
                  borderTypeId: e.target.value ? Number(e.target.value) : null,
                })
              }
              className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
            >
        ...
        <label className="block text-xs text-swu-muted">
          ...
          <select
            value={
              selectedField.passableOverride === null
                ? ''
                : String(selectedField.passableOverride)
            }
            onChange={(e) => {
        ...
        <label className="block text-xs text-swu-muted">
          ...
          <input
            value={(selectedField.effects ?? []).join(', ')}
            onChange={(e) =>
              setSelectedField({
                ...selectedField,
        ...
        <label className="block text-xs text-swu-muted">
          ...
          <input
            ...
            onChange={(e) =>
              setSelectedField({
                ...selectedField,
                adminRegionKey: e.target.value,
              })
            }
            onBlur={(e) =>
              void updateField(selectedField.id, {
                adminRegionKey: e.target.value || null,
              })
            }
            className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
          />
        ...
        {!selectedField.starSystem ? (
              ...
              <label className="block text-xs text-swu-muted">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateSystemForm({
                      ...createSystemForm,
                      systemTypeId: Number(e.target.value),
                    })
                  }
                  className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
                >
          ...
          <div className="space-y-3 rounded border border-swu-border/60 bg-swu-bg/40 p-3">
            ...
            <div className="text-xs text-swu-muted">
              Groesse: {selectedField.starSystem.maxX}x
            ...
            <label className="block text-xs text-swu-muted">
              ...
              <select
                ...
                onChange={(e) =>
                  setSystemGridFieldTypeId(Number(e.target.value))
                }
                className="mt-1 w-full rounded border border-swu-border bg-swu-bg px-3 py-2 text-sm text-swu-text"
              >
                {fieldTypes.map((ft) => (
