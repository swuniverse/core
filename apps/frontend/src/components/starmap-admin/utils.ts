import type { StarmapFieldTypeDto, StarmapGalaxyFieldDto } from '@swuniverse/shared';
...
export function getFieldTypeClasses(fieldType: StarmapFieldTypeDto | null | undefined): string {
  if (!fieldType) return 'bg-swu-bg border-swu-border/30 text-swu-muted';
  return FIELD_TYPE_COLORS[fieldType.key] || 'bg-swu-bg border-swu-border text-swu-primary';
}

export function getGalaxyFieldClasses(field: StarmapGalaxyFieldDto, isSelected: boolean): string {
  const seeded = field.systemTypeId !== null && !field.starSystemId;
  const generated = Boolean(field.starSystemId);

  return [
    'h-7 rounded border text-[10px] font-medium transition relative',
