...
import { spaceBackgroundTile, systemTypeImage, starWarsMarkerImage, planetThumbnail, starTileImage } from '../../lib/assets';
...
export const StarmapCanvas = forwardRef<StarmapCanvasHandle, StarmapCanvasProps>(function StarmapCanvas({
  layer,
  fields,
  routes,
  selectedSystem,
  ...
  // --- Animation helper ---
  const animateTo = useCallback((targetX: number, targetY: number, targetScale: number, duration: number, onComplete?: () => void) => {
  ...
  const drawOverlay = useCallback(() => {
    ...
    const maxCols = state.mode === 'system' && currentGrid ? currentGrid.system.maxX : layer.width;
    const maxRows = state.mode === 'system' && currentGrid ? currentGrid.system.maxY : layer.height;

    const pixelPerCell = state.scale * cellSize;

    // Axis labels only at sufficient zoom
    if (pixelPerCell >= 4) {
      const labelStep = pixelPerCell < 14 ? Math.ceil(14 / pixelPerCell) * 2
        : pixelPerCell < 28 ? Math.ceil(28 / pixelPerCell)
        : 1;

      const startX = Math.max(1, Math.floor(state.viewX / cellSize) + 1);
      const endX = Math.min(maxCols, Math.ceil((state.viewX + (w - AXIS_SIZE) / state.scale) / cellSize));
      const startY = Math.max(1, Math.floor(state.viewY / cellSize) + 1);
      const endY = Math.min(maxRows, Math.ceil((state.viewY + (h - AXIS_SIZE) / state.scale) / cellSize));
      ...
      for (let x = startX; x <= endX; x++) {
        ...
        const screenX = AXIS_SIZE + ((x - 1) * cellSize + cellSize / 2 - state.viewX) * state.scale;
      ...
      for (let y = startY; y <= endY; y++) {
        ...
        const screenY = AXIS_SIZE + ((y - 1) * cellSize + cellSize / 2 - state.viewY) * state.scale;
    ...
    if (sector && state.mode === 'galaxy') {
      ...
      const sx = AXIS_SIZE + (sector.x * sectorSize * CELL_SIZE - state.viewX) * state.scale;
      const sy = AXIS_SIZE + (sector.y * sectorSize * CELL_SIZE - state.viewY) * state.scale;
    ...
    if (field && state.mode === 'galaxy') {
      const fx = AXIS_SIZE + ((field.cx - 1) * CELL_SIZE - state.viewX) * state.scale;
      const fy = AXIS_SIZE + ((field.cy - 1) * CELL_SIZE - state.viewY) * state.scale;
  ...
  const updateView = useCallback(() => {
    ...
    const state = stateRef.current;
    const container = state.mode === 'system' ? systemContainerRef.current : galaxyContainerRef.current;
    ...
    const mapW = isSystem ? sysGrid.system.maxX * SYSTEM_CELL_SIZE : layer.width * CELL_SIZE;
    const mapH = isSystem ? sysGrid.system.maxY * SYSTEM_CELL_SIZE : layer.height * CELL_SIZE;
    if (mapW <= viewW) {
    ...
    if (gridRef.current) {
      gridRef.current.visible = showGridRef.current && state.scale * CELL_SIZE >= 5;
    }
  ...
  const drawSelection = useCallback(() => {
    ...
    const field = fields.find(
      (f) => f.starSystem && f.starSystem.cx === selectedSystem.cx && f.starSystem.cy === selectedSystem.cy,
    );
    if (!field) return;
    ...
    g.rect(x + lw / 2, y + lw / 2, CELL_SIZE - lw, CELL_SIZE - lw)
      .stroke({ color: 0xf59e0b, width: lw, alpha: 0.9 });
  ...
  const drawRoutes = useCallback(() => {
    ...
    for (const route of routes) {
      ...
      for (const segment of route.segments) {
        ...
        g.moveTo((points[0].x - 0.5) * CELL_SIZE, (points[0].y - 0.5) * CELL_SIZE);
        for (let i = 1; i < points.length; i++) {
          g.lineTo((points[i].x - 0.5) * CELL_SIZE, (points[i].y - 0.5) * CELL_SIZE);
        }
  ...
  const drawSectorGrid = useCallback(() => {
    ...
    for (let x = 1; x <= cols; x++) {
      g.moveTo(x * sectorSize * CELL_SIZE, 0).lineTo(x * sectorSize * CELL_SIZE, layer.height * CELL_SIZE);
    }
    for (let y = 1; y <= rows; y++) {
      g.moveTo(0, y * sectorSize * CELL_SIZE).lineTo(layer.width * CELL_SIZE, y * sectorSize * CELL_SIZE);
    }
    g.stroke({ color: 0xf59e0b, width: 1, alpha: 0.12 });
  }, [layer]);

  // --- Transition to system ---
  const transitionToSystem = useCallback((system: StarmapSystemListItemDto) => {
    const state = stateRef.current;
    ...
    const targetX = (system.cx - 1) * CELL_SIZE + CELL_SIZE / 2 - viewW / (2 * targetScale);
    const targetY = (system.cy - 1) * CELL_SIZE + CELL_SIZE / 2 - (app.screen.height - AXIS_SIZE) / (2 * targetScale);
  ...
  const loadSystemTiles = useCallback(async (container: Container) => {
    ...
    const primaryStar = starObjects.find((o) => o.classId === 9001) ?? starObjects[0];
    const secondaryStar = starObjects.find((o) => o.classId === 9002) ?? starObjects.find((o) => o.id !== primaryStar?.id);
    ...
    bg.rect(0, 0, sys.maxX * SYSTEM_CELL_SIZE, sys.maxY * SYSTEM_CELL_SIZE).fill({ color: 0x000000 });
    ...
    const loadPromises = sysFields.map(async (field) => {
      ...
      try {
        ...
        g.rect(cellX, cellY, SYSTEM_CELL_SIZE, SYSTEM_CELL_SIZE).fill({ color: 0x0a0a1a });
        bgLayer.addChild(g);
      }
      ...
      let rendered = false;
      if (starConfig) {
        const tileId = getStarTileIdAt(starConfig.primary, field.sx, field.sy, primaryCenter.x, primaryCenter.y)
          ?? (starConfig.secondary ? getStarTileIdAt(starConfig.secondary, field.sx, field.sy, secondaryCenter.x, secondaryCenter.y) : null);
        if (tileId !== null) {
          try {
            const img = new Image();
      ...
      if (!rendered && field.celestialObjectId) {
        ...
        if (obj?.classId != null && !(obj.classId >= 9001 && obj.classId <= 9005)) {
          try {
            const img = new Image();
            img.crossOrigin = 'anonymous';
            await new Promise<void>((resolve, reject) => {
    ...
    for (let x = 0; x <= sys.maxX; x++) {
      gridGfx.moveTo(x * SYSTEM_CELL_SIZE, 0).lineTo(x * SYSTEM_CELL_SIZE, sys.maxY * SYSTEM_CELL_SIZE);
    }
    for (let y = 0; y <= sys.maxY; y++) {
      gridGfx.moveTo(0, y * SYSTEM_CELL_SIZE).lineTo(sys.maxX * SYSTEM_CELL_SIZE, y * SYSTEM_CELL_SIZE);
    }
  ...
  const loadGalaxyTiles = useCallback(async (mapContainer: Container) => {
    const bgLayer = new Container();
    const iconLayer = new Container();
    ...
    const bg = new Graphics();
    bg.rect(0, 0, layer.width * CELL_SIZE, layer.height * CELL_SIZE).fill({ color: 0x000000 });
    bgLayer.addChild(bg);
    ...
    const loadPromises = fields
      ...
      .map(async (field) => {
        ...
        if (field.systemTypeId) {
          try {
            const iconUrl = field.starSystem?.isMapOnly
              ? starWarsMarkerImage(field.starSystem.landmarkKey, field.systemTypeId)
            ...
            const iconSize = field.starSystem?.isMapOnly ? CELL_SIZE * 0.5 : CELL_SIZE * 0.8;
            iconSprite.width = iconSize;
            iconSprite.height = iconSize;
            iconSprite.position.set(cellX + (CELL_SIZE - iconSize) / 2, cellY + (CELL_SIZE - iconSize) / 2);
        ...
        if (field.fieldType.key === 'NEBULA') {
          const g = new Graphics();
          g.rect(cellX, cellY, CELL_SIZE, CELL_SIZE).fill({ color: 0x10b981, alpha: 0.15 });
          overlayLayer.addChild(g);
        } else if (field.fieldType.key === 'ASTEROID_FIELD' || field.fieldType.key === 'ASTEROID_CLUSTER') {
          const g = new Graphics();
          g.rect(cellX, cellY, CELL_SIZE, CELL_SIZE).fill({ color: 0x78716c, alpha: 0.12 });
          overlayLayer.addChild(g);
        }
      });
    ...
    for (let x = 0; x <= layer.width; x++) {
      grid.moveTo(x * CELL_SIZE, 0).lineTo(x * CELL_SIZE, layer.height * CELL_SIZE);
    }
    for (let y = 0; y <= layer.height; y++) {
      grid.moveTo(0, y * CELL_SIZE).lineTo(layer.width * CELL_SIZE, y * CELL_SIZE);
    }
    grid.stroke({ color: 0xffffff, width: 0.5, alpha: 0.25 });
    mapContainer.addChild(grid);
  }, [fields, layer]);

  const fitViewFn = useCallback((app: Application) => {
    const state = stateRef.current;
    const viewW = app.screen.width - AXIS_SIZE;
    const viewH = app.screen.height - AXIS_SIZE;
    const grid = systemGridRef.current;
    const isSystem = state.mode === 'system' && grid;
    const mapW = isSystem ? grid.system.maxX * SYSTEM_CELL_SIZE : layer.width * CELL_SIZE;
    const mapH = isSystem ? grid.system.maxY * SYSTEM_CELL_SIZE : layer.height * CELL_SIZE;
    const minScale = isSystem ? 0.5 : MIN_SCALE;
    state.scale = clamp(Math.min(viewW / mapW, viewH / mapH), minScale, MAX_SCALE);
    state.viewX = 0;
    state.viewY = 0;
    updateView();
  }, [layer, updateView]);
  ...
  const setupInteraction = useCallback((app: Application, signal: AbortSignal) => {
    ...
    canvas.addEventListener('pointermove', (e) => {
      ...
      if (state.mode === 'galaxy') {
        const worldX = state.viewX + (state.mouseX - AXIS_SIZE) / state.scale;
        const worldY = state.viewY + (state.mouseY - AXIS_SIZE) / state.scale;
        const prev = state.hoveredField;
        state.hoveredField = getFieldAt(worldX, worldY);
        if (state.hoveredField !== prev) onFieldHoverRef.current?.(state.hoveredField);
        updateTooltip();
      }
    }, { signal });
  ...
  const handleClick = useCallback((e: PointerEvent, app: Application) => {
    const state = stateRef.current;
